...
const char* Token::token_names[34] = {
  ...
  "ENDWHILE", "ERR", "END", "VAR", "TRUE", "FALSE", "AND", "OR", "FOR", "ENDFOR","COLON"};
...
Scanner::Scanner(string s):input(s),first(0),current(0) {
  ...
  reserved["do"] = Token::DO; // DO ya existe
...
Token* Scanner::nextToken() {
  ...
  if (isdigit(c)) {
    ...
    case ':': token = new Token(Token::COLON); break; // Añadimos el operador COLON
...
Stm* Parser::parseStatement() {
  ...
  if (match(Token::ID)) {
    ...
    if (!match(Token::ID)) {
      parserError("esperaba un ID después de 'for'");
    }
    string varName = previous->lexema;
    if (!match(Token::COLON))
      parserError("Esperaba ':' 'COLON'");
    Exp* startExp = parseExp();
    if (!match(Token::COMMA))
	    parserError("Esperaba ',' COMMA en el separador del for");
    Exp* endExp = parseExp();
      if (!match(Token::DO)) {
        parserError("Esperaba 'do' después de la expresión de fin");
        exit(0);
      }
      Body* body = parseBody();
      if (!match(Token::ENDFOR)) {
        parserError("Esperaba 'endfor' después del cuerpo del bucle");
        exit(0);
      }
      s = new ForStatement(varName, startExp, endExp, body);
  }
  else {
...
Exp* Parser::parseFactor() {
  ...
  if (match(Token::TRUE)) {
    return new BExp(true);
  }
  if (match(Token::FALSE)) {
    return new BExp(false);
  }
  if (match(Token::LPAREN)) {
    Exp* e = parseExp();
    if (!match(Token::RPAREN))
