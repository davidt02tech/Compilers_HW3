...
enum BinaryOp { PLUS, MINUS, MULT, DIV, EXP, LT, LTEQ, EQ, AND, OR}; // AGREGAR AND OR ?????
...
public:
  ...
  ImpType accept(TypeVisitor* v) override;
  ~BExp();
};
...
public:
  ...
  ForStatement(string var, Exp* start, Exp* end, Body* b);
  int accept(ImpVisitor* v);
  void accept(TypeVisitor* v);
  ~ForStatement();
};
